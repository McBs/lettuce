...
parser.add_argument("--lateral_walls", default='periodic',
                    help="boundary condition in y direction (periodic, bounceback, slip)")
...
if domain_height_in_D % 2 == 0 and gridpoints_per_diameter % 2 != 0:
    ...
    print("(!) domain_height_in_D (DpY) is even, gridpoints_per_diameter will be " + str(
        gridpoints_per_diameter) + ". Use odd domain_height_in_D to enable use of odd GPD!")
...
# lattice
lattice = lt.Lattice(lt.D2Q9, cuda_device, dtype=torch.float32 if Precision == "Single" else torch.float64)
stencil_choice = "D2Q9"
...
### calculate and export 2D obstacle_mask as .png
draw_circular_mask(lattice, gridpoints_per_diameter, output_data=output_data, filebase=output_path + dir_name,
                   print_data=False)
...
sum_time = sim.time_avg["time_collision"] + sim.time_avg["time_store_f_collided"] + sim.time_avg["time_streaming"] + \
           sim.time_avg["time_boundary"] + sim.time_avg["time_reporter"]

print("collision avg. time:", sim.time_avg["time_collision"],
      "seconds (" + str(round(100 * c_time / sum_time, 2)) + " %)")
print("f_c_store avg. time:", sim.time_avg["time_store_f_collided"],
      "seconds (" + str(round(100 * fc_time / sum_time, 2)) + " %)")
