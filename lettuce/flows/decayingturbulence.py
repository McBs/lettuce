...
class DecayingTurbulence:
    ...
    def _generate_initial_velocity(self, ek, wavenumber):
        ...
        e_kin = [np.sum(u_real_h[dim] ** 2 + u_imag_h[dim] ** 2) for dim in range(self.units.lattice.D)]
        ...
        # b = np.zeros(self.dimensions)
        # for dim in range(self.units.lattice.D):
        #     a += wavenumber_modified[dim] * u_real[dim]
        #     b += wavenumber_modified[dim] * u_imag[dim]

        e_kin = [np.sum(u_real_h[dim] ** 2 + u_imag_h[dim] ** 2) for dim in range(self.units.lattice.D)]
        e_kin = np.sum(e_kin) * .5
        print(e_kin)
        e_kin = np.array([(u_real_h[dim] ** 2 + u_imag_h[dim] ** 2) for dim in range(self.units.lattice.D)]).sum(0)
        e_kin = np.mean(e_kin) * .5
        print(e_kin)
        u = np.asarray([
            (np.fft.ifftn(u_real_h[dim] + u_imag_h[dim] * 1.0j, axes=tuple((np.arange(self.units.lattice.D)))) * norm).real
            for dim in range(self.units.lattice.D)])
        e_kin = [np.sum(u[dim] ** 2) for dim in range(self.units.lattice.D)]
        e_kin = np.sum(e_kin) * .5
        print("e_kin pu", e_kin*dx**3)
        e_kin = np.array([(u[dim] ** 2) for dim in range(self.units.lattice.D)]).sum(0)
        e_kin = np.mean(e_kin) * .5
        print(e_kin*dx**3)
        return u
