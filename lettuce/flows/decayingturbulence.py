...
class ForcedTurbulence:
    ...
    def _generate_spectrum(self):
        ...
        # ek /= np.sum(ek)
        # ek *= self.ic_energy
        ek = 16/np.sqrt(0.5*np.pi)*self.u_rms*wavenorms**4/self.k0**5*np.exp(-2*wavenorms**2/self.k0**2)
        self.spectrum = ek[..., None] * wavemask
        self.spectrum = np.sum(self.spectrum, axis=tuple((np.arange(self.units.lattice.D))))
        return ek, wavenumber
    ...
    def _generate_initial_velocity(self, ek, wavenumber):
        ...
        u = np.asarray([
            (np.fft.ifftn(u_real[dim] + u_imag[dim] * 1.0j, axes=tuple((np.arange(self.units.lattice.D))), norm="forward")).real
            for dim in range(self.units.lattice.D)])
        rms = np.sqrt((u[0]**2+u[1]**2+u[2]**2).mean()/3)
        uu = u * self.u_rms/rms

        print(np.sqrt((uu[0]**2+uu[1]**2+uu[2]**2).mean()/3))

        return u * self.u_rms/rms
