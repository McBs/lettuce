...
from lettuce.boundary import newsuperTGV3D, superTGV2D
...
class ReducedTaylorGreenVortex2D:
    def __init__(self, resolution, reynolds_number, mach_number, lattice):
        ...
        self.units = UnitConversion(
            lattice,
            reynolds_number=reynolds_number, mach_number=mach_number,
            characteristic_length_lu=resolution, characteristic_length_pu=np.pi/2,
            characteristic_velocity_pu=1
        )
    ...
    @property
    def grid(self):
        x, dx = np.linspace(np.pi/2,np.pi, num=(self.resolution), endpoint=False, retstep=True)
        x = x + dx / 2
        y, dy = np.linspace(np.pi/2,np.pi, num=(self.resolution), endpoint=False, retstep=True)
        y = y + dy / 2
        return np.meshgrid(x, y, indexing='ij')

    @property
    def boundaries(self):
        boundary = superTGV2D(lattice=self.units.lattice)
        return [boundary]
