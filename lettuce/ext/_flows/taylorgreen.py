        ...
        if self.dist == "mpi":
            ...
            if remainder > 0:
                ...
                for i in range(remainder):
                    ...
                    right_neighbor = bigsplits[i+1][:upperfill_big] if i < remainder - 1 else smallsplits[0][:upperfill_small]  # Get first value of next (or first split for last one)
                ...
                for i in range(len(range(remainder, dist.get_world_size()))):
                    ...
                    right_neighbor = smallsplits[i+1][:upperfill_small] if i < len(range(remainder, dist.get_world_size())) - 1 else bigsplits[0][:upperfill_big]  # Get first value of next (or first split for last one)
